...
@section('content')
    <div class="chat-wrapper">
        ...
        <div class="chat-main card">
            ...
            {{-- Messages --}}
            <div class="chat-message-list" style=" overflow-y:auto;">
                @include('admin.tickets.partials.messages', ['messages' => $ticket->messages])
            </div>
            ...
            {{--                            @endif--}}
            {{--                            <p class="chat-time mb-0"><span >{{ $msg->created_at->format('h:i A') }}</span></p>--}}
            {{--                        </div>--}}
            {{--                    </div>--}}
            {{--                @else--}}
            ...
            {{--                                <a href="{{ asset('storage/'.$msg->attachment_path) }}" target="_blank">📎 Attachment</a>--}}
            {{--                            @endif--}}
            {{--                            <p class="chat-time mb-0"><span style="float:inline-end">{{ $msg->created_at->format('h:i A') }}</span></p>--}}
            {{--                        </div>--}}
            {{--                    </div>--}}
            {{--                @endif--}}
            {{--            @endforeach--}}
            ...
            @if($ticket->status_id == 1)
                <form id="chatMessageForm" action="{{ route('admin.tickets.messages.store', $ticket->id) }}" method="POST" enctype="multipart/form-data" class="chat-message-box  mt-5">
                    @csrf
...
@push('scripts')
    <script>
        ...
        function fetchMessages() {
            ...
            $.ajax({
                url: "{{ route('admin.tickets.messages.fetch', $ticket->id) }}",
                type: "GET",
                success: function (data) {
                    $(".chat-message-list").html(data);
                    // auto scroll bottom
                    var chatList = $(".chat-message-list");
                    chatList.promise().done(function(){
