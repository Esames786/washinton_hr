...
trait AttendanceServiceTrait_copy
{

    /**
     * Strip seconds from a time string (e.g. 09:15:45 → 09:15:00)
     */
    private function stripSeconds(string $time): string
    {
...
//        $shiftEndTs   = strtotime($attendance_date . ' ' . $shift->shift_end);
        $shiftStartTs = strtotime($attendance_date . ' ' . $this->stripSeconds($shift->shift_start));
        $shiftEndTs   = strtotime($attendance_date . ' ' . $this->stripSeconds($shift->shift_end));
        ...
        if ($check_in_time) {
            ...
            $check_in_time = $this->stripSeconds($check_in_time);
        ...
        if ($check_out_time) {
            ...
            $check_out_time = $this->stripSeconds($check_out_time);
    ...
    {
        $timestamp  = strtotime($attendance_date . ' ' . $this->stripSeconds($check_in_time));
        $shiftStart = strtotime($attendance_date . ' ' . $this->stripSeconds($shift->shift_start));
        $shiftEnd   = strtotime($attendance_date . ' ' . $this->stripSeconds($shift->shift_end));
...
//        $shiftEnd   = strtotime($attendance_date . ' ' . $shift->shift_end);
        ...
        for ($i = 0; $i < count($timeRules); $i++) {
            ...
            $currentTime = strtotime($attendance_date . ' ' . $this->stripSeconds($currentRule->entry_time));
            if ($shiftEnd <= $shiftStart) {
                $currentTime += 24 * 3600;
            }

            $nextTime = $i + 1 < count($timeRules)
                ? strtotime($attendance_date . ' ' . $this->stripSeconds($timeRules[$i + 1]->entry_time))
                : $shiftEnd;
    ...
    {
        $cutoff = $this->stripSeconds($cutoff);
    ...
    public function calculateCheckOutStatus($check_out_time, $attendance, $shift, $rules, float $basic_salary): array
    {
        ...
        // Original timestamps (seconds preserved)
        ...
        // Priority: Early Quarter → Early Halfday → Early Exit
        ...
        // ----- Minute-level comparison (ignore seconds) -----
        ...
        foreach ($earlyRules as $r) {
            ...
            // Overnight adjust cutoff if needed
            if ($shiftEndTs <= $shiftStartTs && $cutoffMinuteTs < $shiftStartTs) {
                $cutoffMinuteTs += 24 * 3600;
            }
            ...
            if ($checkOutMinuteTs <= $cutoffMinuteTs) {
                ...
                // Early Quarter / Early Halfday override
                return [
...
////        $checkInTs  = strtotime($attendanceDate . ' ' . $attendance->check_in);
////        $checkOutTs = strtotime($attendanceDate . ' ' . $check_out_time);
//        $checkInTs  = strtotime($attendanceDate . ' ' . $this->stripSeconds($attendance->check_in));
//        $checkOutTs = strtotime($attendanceDate . ' ' . $this->stripSeconds($check_out_time));
//
//        $shiftStartTs = strtotime($attendanceDate . ' ' . $this->stripSeconds($shift->shift_start));
//        $shiftEndTs   = strtotime($attendanceDate . ' ' . $this->stripSeconds($shift->shift_end));
//
//        // Overnight adjustment for checkout
//        if ($shiftEndTs <= $shiftStartTs) {
//            $shiftEndTs += 24 * 3600;
//
//            if ($checkOutTs < $shiftStartTs) {
...
//            // Agar shift overnight hai to cutoff bhi adjust karo
//            if ($shiftEndTs <= $shiftStartTs && $cutoffTs < $shiftStartTs) {
//                $cutoffTs += 24 * 3600;
//            }
//
//            dd($checkOutTs,$cutoffTs );
//
...
//    }
    ...
    private function combineDeductPercentsIfApplicable(
        ...
        $deductableStatuses = [1,2, 3, 9]; // Late, Half Day, Quarter
